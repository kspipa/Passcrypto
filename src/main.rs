...
fn right(ui : &mut Cursive, passs : &mut HashMap<Vec<u8>, i8>, key : Vec<u8>){
    ...
    let dialog = Dialog::around(LinearLayout::vertical()
        ...
        .child(ResizedView::with_fixed_size((5, 2), Button::new("Delete", move |g| {delete_pass(g, &mut _ll.clone(), key.clone())})))
...
fn delete_pass(s: &mut Cursive, passs : &mut HashMap<Vec<u8>, i8>, key : Vec<u8>) {
